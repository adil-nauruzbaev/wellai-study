...
extension DioResponseUtils on Response {
  Map<String, dynamic> get verbose {
    ...
    final reqEndpoint = '${requestOptions.method} ${requestOptions.baseUrl}';
    final endpointsMatch = _match(endpoint, reqEndpoint);
    final output = <String, dynamic>{
      'endpoint': endpoint,
      if (!endpointsMatch) 'redirect': '${requestOptions.method} ${requestOptions.baseUrl}${requestOptions.path}',
...
bool _match(String endpoint, String redirect) {
  final length = switch (endpoint.length > redirect.length) {
    true => redirect.length,
    false => endpoint.length,
  };

  final path1 = endpoint.substring(0, length);
  final path2 = redirect.substring(0, length);
  return path1 == path2;
}
...
dynamic _shortenObject(dynamic object) {
  if (object is String) {
    ...
    return object.map(_shortenObject).toList();
  } else {
    return object;
  }
}
